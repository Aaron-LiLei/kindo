...
孩子对着电视说一句「我想看汪汪队」，就能播放家里收藏的动画片、儿歌和故事；到了约定的观看时间，「成长接力」会温柔地接住孩子的兴趣；家长用浏览器管理内容、设定规则。同样的体验也有触屏版：家里的 Android 平板（7~12 寸）装上就能用，和电视端同一套口径。
...
- 不用键盘不用识字：界面是图标和大按钮，颜色明快、焦点醒目；电视用遥控器、平板用手指点；「找一找」页可以按类型和主题挑选
...
| 首页 | 内容墙：自适应 7~12 寸与横竖屏 |
|---|---|
| ![平板端首页](docs/images/pad_home.jpg) | ![平板内容墙](docs/images/pad_browse.jpg) |
...
| Android TV | 电视或盒子（Android 7.0+，arm/arm64；遥控器最好带麦克风键） |
| Android 平板（可选） | 7~12 寸，Android 8.0+，与电视端同一套 Hub 和规则 |
...
3. 在电视 / 平板上安装对应端的 APK → 输入 Hub 地址 → 输入 6 位配对码 → 在后台「设备/配对」批准

> 语音识别模型首次部署需下载（约 230MB，详见下方「语音识别模型」）。家长声音克隆为可选能力，由独立的 kindo-tts 容器提供：构建前按 [apps/kindo-tts/README.md](apps/kindo-tts/README.md) 放置模型即可启用；模型缺失时自动回退系统语音，不需要该功能时也可在 compose 中整段注释掉该服务。Kindo 是局域网产品，请勿直接暴露到公网；需要远程管理时走 VPN 或 HTTPS 反向代理（见 [SECURITY.md](SECURITY.md)）。
...
| `apps/kindo-tts` | 可选：家长声音克隆 TTS 服务（sherpa-onnx ZipVoice，纯 CPU 离线） |
