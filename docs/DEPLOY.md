...
1. 打开 `http://<主机>:8090/admin` 初始化管理员。未预设 `KINDO_ADMIN_BOOTSTRAP_TOKEN`
   时，Hub 自动生成一次性 token 写入 `data/hub/bootstrap/ADMIN_BOOTSTRAP_TOKEN`
   （设置密码后立即作废）。
2. LLM Provider 二选一：推荐「AI 模型」页录入（Key 落盘加密）；或解开
   `config/kindo.yaml` 中 `llm_providers` 段注释并在 `.env` 提供三个 `KINDO_LLM_*`
   变量（`kindo.example.yaml` 中该段默认注释——原样复制且未设变量会因占位符
   无法解析导致 Hub 启动失败）。
3. 「媒体库 → 来源与扫描」录入媒体来源：本地目录在容器内路径为 `/media/family`
   （即 .env 的 MEDIA_DIR 只读挂载）；SMB/WebDAV 凭据在页面录入（落盘加密）。
4. TV/Pad 安装 Release 附带 APK（TV 需 Android 7.0+，Pad 需 Android 8.0+）→ 配对码绑定。
...
- 首次下载期间 kindo-asr 显示 starting（慢网下可达数十分钟）属预期；若
  `docker compose up -d` 曾报 "dependency failed to start"，模型就绪后模型卷已持久，
  再执行一次 `docker compose up -d` 即可拉起 hub。
...
| 升级 | 升级前建议备份 `data/hub/kindo.db` → `.env` 改 `KINDO_VERSION` → `docker compose pull && docker compose up -d` |
| 数据 | `data/hub/`（SQLite/配置/样本）与模型卷均持久；数据库迁移随 Hub 启动自动执行 |
| 模型 | 卷持久 + 清单校验，升级不重复下载；模型版本升级随镜像清单自动增量替换 |
| 回滚 | `.env` 改回旧版本号 → `docker compose up -d`。若曾运行过更新版本，数据库迁移可能已推进，旧版本无法回退迁移——先恢复升级前的 `kindo.db` 备份再回滚 |
